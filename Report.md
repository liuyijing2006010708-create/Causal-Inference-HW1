...
* **关键 Prompt**：“我写了围绕这6个问题的大致理论解释，里面包含了我对模拟场景A、B、C、D的一些初步思考。请帮我用更规范、更有学术深度的计量经济学语言进行润色。以下是我的解释：
  1. PSM有效是因为条件独立假设。如果选择只看可观测变量X，那控制了X之后处理组和控制组就一样了。得分能降维。
  2. DID能解决不随时间变化的漏掉变量（固定效应），政策前后做个减法就消掉了，像场景A那样。
  3. 如果趋势和X有关，两组的X分布不一样，单纯DID就会把趋势差异当成政策效应（场景B）。PSM-DID把特征配对对齐，就恢复了平行趋势。
  4. 如果漏掉的变量随时间变（比如场景C的冲击），PSM看不见它，DID做差分也减不掉，所以方法全不灵。
  5. 匹配肯定会变对象，因为我们的卡尺和不放回匹配是以处理组为准，丢了很多控制组。所以算出来的是ATT而不是ATE。
  6. 共同支持不够的话，不匹配会有外推偏误；匹配的话要删样本，样本少了方差就会变大。”
* **AI 初始输出**：AI接收草稿后进行了学术化扩写，但在回答第6问时，给出了如下表述：“在共同支持不足时，估计量的方差会显著恶化。例如在我们的模拟中，场景 D 中 PSM-DID 的标准差（0.1200）远远大于场景 A 中全样本DID的标准差（0.0070），这强有力地证实了共同支持区域不足会带来剧烈的效率损失。”
* **人工判断与修改**：我在审查AI的输出时，发现了这个逻辑漏洞。场景 A（完全平行趋势且无选择偏误）和场景 D（非线性自选择且共同支持不足）属于完全不同的数据生成机制，两者的对照基准完全不同。不能拿场景D匹配后的方差去跟场景A的理想全样本方差做横向跨场景对比。正确的逻辑应该是在场景D内部进行纵向对比（对比 `DID_Basic` 的 0.0806 与 `PSM-DID` 的 0.1200），从“强制裁剪离群样本导致有效容量 $N$ 萎缩”以及“权重过度集中”等内部数理机制来解释方差的恶化。因此，我向AI发出明确的纠偏指令，要求其修正对比基准并重写第6问。
