...
通过观察Jitter抖动图可以直观发现，未匹配全样本中处理组与控制组的倾向得分分布存在极其显著的非对称性。在施加了 `discard = "both"` 条件后，算法精准斩断并丢弃了非重叠区间内的非可比样本（1372名控制组样本与2名处理组样本因不具备可比性被剔除 ）。这确保了匹配后的 302 对样本共享高度重叠的共同支持区域，在根源上免受参数外推偏误的侵蚀。
